...
<body onload="getUserData(); getAllSubServices()">
...
<div id="serviceInfo" class="well well-large"
     style="display: none;justify-content: center;align-items: center;background-color: #48ddf1;width: 85%;height:5%;margin-top: 1%;margin-left: 5%">
</div>
...
<script>
    ...
    function getServiceId(){
        ...
        serviceId = params.toString().split("=")[1];
    }
    ...
    function getAllSubServices() {
        ...
        $.ajax({
            type: "GET",
            url: "/getSubServiceByServiceId/" + serviceId,
            success: function (response) {
                if(response.length > 0) {
                    $("#serviceInfo").html("Service : " + response[0].service.name);
                    $("#serviceInfo").show();
                    createSubServiceDivs(response);
                }else {
                    $("#serviceInfo").html("Service has no sub service");
                    $("#serviceInfo").show();
                }
            }
        });
    ...
    function createSubServiceDivs(response) {
